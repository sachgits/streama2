...
class UserController {
  ...
  def saveAndCreateUser() {
    ...
    Account userAccnt = kbAccountService.createAccountWithDefaultPaymentMethod(userInstance,"__EXTERNAL_PAYMENT__");
        def result = [:]
        if(userAccnt == null){
          result.message ="unable to create kb user account please try again later "
          response.setStatus(NOT_ACCEPTABLE.value)
          respond result
          return
        }
