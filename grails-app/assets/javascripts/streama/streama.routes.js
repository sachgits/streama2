...
angular.module('streama').config(function ($stateProvider) {
	...
	$stateProvider
        ...
        .state('subscription',{
            url: '/subscription',
			templateUrl: '/streama/subscrib2.htm',
			controller: 'KbSubscriptionCtrl',
            resolve: {
				currentUser: resolveCurrentUser
			}
        })
    ...
    function checkUserSubscription(kbSubscriptionService,$rootScope,$state) {
        if(kbSubscriptionService.isUserEntitledToService()){
            return resolveCurrentUser(apiService,$rootScope);
        }else{
